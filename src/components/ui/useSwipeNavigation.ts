...
export const useSwipeNavigation = <T extends string>({
  ...
  const updateTrack = useCallback(
    (offset: number | string, animate: boolean) => {
      ...
      track.style.transition = animate
        ...
        : 'none'
      track.style.transform = `translate3d(${typeof offset === 'number' ? `${offset}px` : offset}, 0, 0)`
    },
    [],
  )
  ...
  const handleSwiped = useCallback(
    (swipe: SwipeEventData) => {
      ...
      clearSettleTimer()
      setIsSettling(true)
      // SwipePager defines the same CSS step for panel placement and settling.
      updateTrack(
        direction === 'next'
          ? 'calc(-1 * var(--swipe-step))'
          : 'var(--swipe-step)',
        true,
      )
      onSwipeCommit?.(adjacentKey)
